  ...
  <BasicLayout>
    ...
    <template v-if="orders">
      <div class="order" v-for="order in orders" :key="order.id">
        ...
        <div class="products">
          <div class="product" v-for="product in order.attributes.data.data" :key="product.data.id">
             <div>
              <img
                :src="API_URL_IMAGE + product.data.attributes.image.data[0].attributes.url"
                :alt="product.data.attributes.name"
                class="ui image" 
              />
              <p>{{ product.data.attributes.name }}</p>
            </div> 
            <div>{{ product.data.attributes.quantity }} x {{ product.data.attributes.price }} $</div>
          </div>
        </div>
      </div>
    </template>
